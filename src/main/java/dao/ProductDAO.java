...
public class ProductDAO extends JdbcDAO<Product> implements GenericDAO<Product> {
    ...
    protected static final String USER_ACCOUNT_TABLE = "`login-page-jsp-demo`.`product`";
    ...
    protected static final String INSERT_STATEMENT = "INSERT INTO `login-page-jsp-demo`.`product`\n" +
                                                     "(`CODE`,\n" +
                                                     "`NAME`,\n" +
                                                     "`PRICE`)\n" +
                                                     "VALUES (?, ?, ?);";
    protected static final String UPDATE_STATEMENT = "UPDATE `login-page-jsp-demo`.`product`\n" +
                                                     "SET\n" +
                                                     "`CODE` = ?,\n" +
                                                     "`NAME` = ?,\n" +
                                                     "`PRICE` = ?\n" +
                                                     "WHERE `CODE` = ?;\n";
    protected static final String DELETE_STATEMENT = "DELETE FROM `login-page-jsp-demo`.`product`\n" +
                                                     "WHERE `CODE` = ?;\n";
    protected static final String SELECT_STATEMENT = "SELECT *\n" +
                                                     "FROM `login-page-jsp-demo`.`product`\n" +
                                                     "WHERE `CODE` = ?\n";
...
//                PreparedStatement ps = connection.prepareStatement(INSERT_ERROR_STATEMENT);
            ...
            throw new DuplicateException(e, product.getCode());
        }
    ...
    public Product update(Product product, String previousCode) throws SQLException {
        Product updatedProduct = null;
        try (
                PreparedStatement ps = connection.prepareStatement(UPDATE_STATEMENT);
        ) {
